...
class Diffusion(object):
    ...
    # def create_Hfuncs(self, config, deg):
    #     # deg = self.args.deg
    #     # config = self.config
    #     H_funcs = None
    #     if deg[:2] == "cs":
    ...
    def sample(self, cls_fn=None):
        ...
        for x_orig, classes in pbar:
            self.sample_once(
                x_orig, sigma_0, config, deg, H_funcs, model, idx_so_far, cls_fn, classes
            )

    # 如果沒有cls，就不會有class
    def sample_once(
        self, x_orig, sigma_0, config, deg, H_funcs, model, idx_so_far, cls_fn, classes=None
    ):
    ...
    def sample_image(self, x, y_0, sigma_0, H_funcs, model, last=True, cls_fn=None, classes=None):
